...
| 鼠标悬停 | 在角色上方显示紧凑状态卡；互动栏默认在右侧，右侧空间不足时自动换到左侧，状态卡和二级菜单不得超出当前显示器 |
...
| 心情值 `mood` | 0～100 | 75 | 忽视时缓慢降低；抚摸、不同运动和音乐恢复 |
| 体力值 `energy` | 0～100 | 80 | 清醒时极慢降低，运动明显降低；休息和睡眠恢复 |
...
自然状态默认每 10 分钟统一结算一次；喂食、运动等用户操作立即生效。离线补算最多 72 小时，防止长时间未启动后数值直接归零。令：
...
| 操作 | 数值效果 | 动作 |
| :--- | :--- | :--- |
| 营养餐 | 饱腹 +35、健康 +3、心情 +2、经验 +4 | `088` → `147` |
| 冰淇淋 | 饱腹 +10、心情 +14、健康 -2、清洁 -3、经验 +2 | `162` 连播三次 |
| 抚摸 | 心情 +5、好感 +2 | `134` 或爱心池 |
| 热身弹跳 | 心情 +4、健康 +2、体力 -6、饱腹 -2、清洁 -1、经验 +2 | `108` |
| 啦啦操 | 心情 +10、健康 +3、体力 -14、饱腹 -4、清洁 -2、好感 +2、经验 +3 | `030` 连播两次 |
| 跑步 | 心情 +7、健康 +5、体力 -28、饱腹 -6、清洁 -5、好感 +2、经验 +4 | `179` 连播三次 |
| 洗澡 | 清洁度 +55、心情按偏好 ±3、经验 +3 | `158` → `019` |
| 小睡 | 每分钟体力 +0.2 | `160` |

以上数值为默认值。用户可在“控制面板 → 外观与行为 → 养成参数”中设置 1～60 分钟结算周期、0～5 倍自然消耗、0～2 点/小时体力消耗和 0～4 倍运动体力消耗。
| 治疗 | 健康低于 60 时可用；健康 +25 | `150` 完整播放后恢复 |

同一操作必须有冷却。例如连续抚摸 10 次不能无限增加好感；前 3 次有效，之后进入 5 分钟冷却。
...
喂食和运动在角色侧边悬停按钮列中展开二级选项；按钮列默认在右侧，靠近屏幕右边时自动换到左侧。摸摸、洗澡、睡觉/叫醒和治疗保留为直接按钮，避免右键菜单混合“照顾”与“配置”两类操作。
...
2. 营养餐、冰淇淋、三种运动、抚摸、洗澡、休息和治疗同时改变数值与动作。
3. 连点不能无限刷好感和经验。
4. 等级达到阈值后准确解锁对应动作。
5. 重启后数值、等级和解锁保持一致。
...
当前进度（2026-08-07）：M1、M2 与 M3 已完成并接入正式 EXE；下一阶段进入 M4“环境与智能”。
